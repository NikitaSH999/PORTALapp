...
class PortalTrialActivator {
  ...
  Future<PortalExperience> activateTrial({Locale? locale}) async {
    ...
    if (managedManifest.isConfigured) {
      try {
        final managedProfile = await portalRepository.fetchManagedProfile(
          managedManifest,
        );
        final importContent = managedProfile.importContent();
        if (importContent != null && importContent.trim().isNotEmpty) {
          await profileRepository
              .addLocal(importContent)

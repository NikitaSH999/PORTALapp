...
class ResolvedDirectPackagePreset {
  ...
  final String title;
  final String description;
  final List<InstalledPackageInfo> apps;
...
const directPackageCatalog = <DirectPackageCatalogEntry>[
  ...
  DirectPackageCatalogEntry(
    ...
    title: 'Marketplaces',
    description:
        'Shopping and delivery apps that benefit from fast local paths.',
    packageNames: [
      'ru.ozon.app.android',
      'com.wildberries.ru',
      'ru.yandex.market',
  ...
  DirectPackageCatalogEntry(
    ...
    title: 'Gov & media',
    description:
        'Public services and local media apps worth keeping easy to reach.',
    packageNames: [
...
List<ResolvedDirectPackagePreset> resolveDirectPackagePresets({
  required Iterable<InstalledPackageInfo> installedPackages,
}) {
  ...
  return [
    for (final entry in directPackageCatalog)
      if (entry.packageNames
          .where(installedByPackageName.containsKey)
          .isNotEmpty)

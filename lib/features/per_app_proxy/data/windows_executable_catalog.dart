...
Future<List<InstalledPackageInfo>> discoverWindowsExecutables() async {
  ...
  final sorted = suggestions.values.toList(growable: false)
    ..sort((left, right) {
      ...
      return left.info.name
          .toLowerCase()
          .compareTo(right.info.name.toLowerCase());
    });
...
List<Directory> _candidateRoots() {
  ...
  final roots = <String>{
    if (env['LOCALAPPDATA'] case final localAppData?
        when localAppData.isNotEmpty)
      p.join(localAppData, 'Programs'),
    if (env['LOCALAPPDATA'] case final localAppData?
        when localAppData.isNotEmpty)
      localAppData,
    if (env['PROGRAMFILES'] case final programFiles?
        when programFiles.isNotEmpty)
      programFiles,
    if (env['PROGRAMFILES(X86)'] case final programFilesX86?
        when programFilesX86.isNotEmpty)
      programFilesX86,
    if (env['ProgramW6432'] case final programW6432?
        when programW6432.isNotEmpty)
      programW6432,
  };
...
Future<void> _scanDirectory(
  ...
  if (depth > _maxDirectoryDepth ||
      suggestions.length >= _maxExecutableSuggestions) {
    return;
  }
...
String _displayNameForExecutable(String fileName, String executablePath) {
  ...
  if (parentName.isNotEmpty &&
      !_genericDirectoryNames.contains(parentName.toLowerCase())) {
...
class _ScoredExecutable {
  ...
  final int score;
  final InstalledPackageInfo info;
}
